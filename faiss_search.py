...
def normalize_format(fmt):
    ...
    return " ".join(tokens)  # "video/mp4" → "video mp4"
...
# --- Search Function ---
def search_resources(query, level="matric", style="visual", top_k=5):
    embedding = model.encode([query])
    D, I = index.search(np.array(embedding).astype("float32"), top_k)

    raw_results = [metadata[i] for i in I[0] if i < len(metadata)]
    filtered = [r for r in raw_results if is_relevant_to_student(r, level.lower(), style.lower())]

    return filtered if filtered else raw_results[:top_k]  # fallback to raw top-k

# --- Optional CLI Test ---
if __name__ == "__main__":
    query = "binary number video"
    results = search_resources(query, level="matric", style="visual", top_k=5)
